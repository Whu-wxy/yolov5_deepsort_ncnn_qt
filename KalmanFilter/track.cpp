...
void Track::predit(DPKalmanFilter *kf)
{
    /*Propagate the state distribution to the current time step using a
        ...
        Parameters
        ----------
        kf : kalman_filter.DPKalmanFilter
            The Kalman filter.
        */
...
void Track::update(DPKalmanFilter * const kf, const DETECTION_ROW& detection)
{
...
void Track::featuresAppendOne(const FEATURE &f)
{
    ...
    FEATURESS newfeatures = FEATURESS(size+1, 512); // 128
    newfeatures.block(0, 0, size, 512)/* 128*/ = this->features;
